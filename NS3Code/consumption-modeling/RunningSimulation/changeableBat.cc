...
double getDistToTransmit(){
  std::cout << "\n enter the number of meters between the nodes: ";
  double meters;
  std::cin >> meters; 
  return meters;
}
...
main (int argc, char *argv[])
{
  ...
  double distanceToRx = 100;  // meters
  /*
  ...
  cmd.AddValue ("InitEn", "Initial energy of transceiver", initEn);
  cmd.Parse (argc, argv);
  ...
  basicSourceHelper.Set ("BasicEnergySourceInitialEnergyJ", DoubleValue (initEn));
  // install source
  EnergySourceContainer sources = basicSourceHelper.Install (c);
  /* device energy model */
  WifiRadioEnergyModelHelper radioEnergyHelper;
  // configure radio energy model
  radioEnergyHelper.Set ("TxCurrentA", DoubleValue (0.0000174));
